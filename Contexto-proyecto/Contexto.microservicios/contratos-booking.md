# Contrato de Integración API de Atracciones v2.1 (Booking Público)
...
| `/atracciones/{guid}` | Sí | 5 min | Detalle estable en períodos cortos |
...
| 409 | Conflict | Regla de negocio, por ejemplo cupos insuficientes | Sí (`Error`) |
  ...
  /reservas:
    ...
    post:
      ...
      description: |
        ...
        - **Con JWT** (`ClienteAutenticado`): el `cli_id` se extrae del token; no se requiere en el body.
...
Crear cabecera RESERVAS.
Insertar líneas RESERVA_DETALLE (una por tipo de ticket).
Descontar cupos en HORARIO con la suma de cantidades.
Validar consistencia monetaria (subtotal, iva, total).
...
## 9) Referencia de códigos HTTP por endpoint  
### Implementación API v1

### Convenciones globales

| Origen | Códigos | Cuerpo de error |
|---|---:|---|
| `ValidateModelFilter` + reglas de negocio vía `ValidationException` | 400 | `ApiErrorResponse` en JSON `snake_case` |
| JWT ausente/inválido (`OnChallenge`) | 401 | Misma forma que `ApiErrorResponse` en `snake_case` |
...
| Método y ruta | Éxito | Errores documentados en Swagger |
|---|---|---|
...
| `POST /api/v1/reservas` | 201 + `ApiItemResponse`<br>*(status 201 en envelope)* | 400, 401, 404, 409, 500 |
...
| `POST /api/v1/resenias` | 201 + `ApiItemResponse`<br>*(status 201)* | 400, 401, 403, 404, 409, 500 |
