...
public sealed class AtraccionesDbContextFactory : IDesignTimeDbContextFactory<AtraccionesDbContext>
{
    public AtraccionesDbContext CreateDbContext(string[] args)
    {
        ...
        var connectionString =
            Environment.GetEnvironmentVariable("ConnectionStrings__AtraccionesDb")
            ?? Environment.GetEnvironmentVariable("ConnectionStrings__DefaultConnection")
            ?? "Host=localhost;Port=5432;Database=ms_atracciones_db;Username=postgres;Password=postgres";
    ...
    private static void LoadEnvFile()
    {
        ...
        {
            Path.Combine(currentDirectory, "services", "ms-atracciones", ".env"),
