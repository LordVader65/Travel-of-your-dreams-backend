...
public sealed class IncluyeService : IIncluyeService
    {
        ...
        public async Task<IncluyeResponse> CrearAsync(CrearIncluyeRequest request, CurrentUserData user, CancellationToken cancellationToken = default) { Support.Guard.EnsureAdmin(user); Validate(request); return Map.Incluye(await _data.CrearAsync(Map.Incluye(request), cancellationToken)); }
        ...
        private static void Validate(CrearIncluyeRequest request) { var errors = new List<string>(); Support.Guard.Required(request.Descripcion, "Descripcion", errors); Support.Guard.MaxLength(request.Descripcion, 200, "Descripcion", errors); Support.Guard.MaxLength(request.Tipo, 20, "Tipo", errors); Support.Guard.ThrowIfAny(errors); }
    }
