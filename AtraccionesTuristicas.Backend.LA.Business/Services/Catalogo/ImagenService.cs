...
public sealed class ImagenService : IImagenService
    {
        ...
        public async Task<ImagenResponse> CrearAsync(CrearImagenRequest request, CurrentUserData user, CancellationToken cancellationToken = default) { Support.Guard.EnsureAdmin(user); var errors = new List<string>(); Support.Guard.Required(request.Url, "Url", errors); Support.Guard.ThrowIfAny(errors); return Map.Imagen(await _data.CrearAsync(Map.Imagen(request), cancellationToken)); }
