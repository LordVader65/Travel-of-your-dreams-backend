...
public sealed class AtraccionService : IAtraccionService
    {
        ...
        public async Task<AtraccionResponse> CrearAsync(CrearAtraccionRequest request, CurrentUserData user, CancellationToken cancellationToken = default) { Support.Guard.EnsureAdmin(user); Validate(request.Nombre, request.DestinoId); return Map.Atraccion(await _data.CrearAsync(Map.Atraccion(request), cancellationToken)); }
        public async Task<AtraccionResponse> ActualizarAsync(ActualizarAtraccionRequest request, CurrentUserData user, CancellationToken cancellationToken = default) { Support.Guard.EnsureAdmin(user); Validate(request.Nombre, request.DestinoId); return Map.Atraccion(await _data.ActualizarAsync(Map.Atraccion(request), cancellationToken) ?? throw new NotFoundException("Atraccion no encontrada.")); }
        ...
        private static void Validate(string nombre, int destinoId) { var errors = new List<string>(); Support.Guard.Required(nombre, "Nombre", errors); Support.Guard.Positive(destinoId, "DestinoId", errors); Support.Guard.ThrowIfAny(errors); }
    }
