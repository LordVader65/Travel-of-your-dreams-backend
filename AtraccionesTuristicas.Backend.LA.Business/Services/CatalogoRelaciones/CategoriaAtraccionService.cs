...
public sealed class CategoriaAtraccionService : ICategoriaAtraccionService
    {
        ...
        public async Task<BusinessOperationResult> RemoverAsync(int atraccionId, int categoriaId, CurrentUserData user, CancellationToken cancellationToken = default) { Support.Guard.EnsureAdmin(user); Validate(atraccionId, categoriaId, "CategoriaId"); await _data.RemoverAsync(atraccionId, categoriaId, cancellationToken); return BusinessOperationResult.Ok("Categoria desasociada."); }
